...
public class MessageTests
{
    ...
    public void CreateMessage_ShouldSucceed_WhenDataIsValid(Guid roomId, Guid memberId, string messageContent, DateTime sentAt)
    {
        // Act
        ...
        Assert.Equal(messageContent, result.Value.MessageContent);
        Assert.Equal(sentAt, result.Value.SentAt);
    }

    [Theory]
    [MemberData(nameof(GetInvalidMessageData))]
    public void CreateMessage_ShouldFail_WhenDataIsInvalid(Guid roomId, Guid memberId, string messageContent, DateTime sentAt, List<ValidationError> expectedErrors)
    {
        // Act
        var result = Message.Create(roomId, memberId, messageContent, sentAt);
    ...
    public static IEnumerable<object[]> GetInvalidMessageData()
    {
        ...
        yield return new object[]
        {
            ...
            new List<ValidationError>
            {
                Error.Validation("messageContent cannot be null or empty", "ERR_IS_NULL_OR_EMPTY", "messageContent"),
                Error.Validation("messageContent must be between 1 and 500 characters", "ERR_LENGTH_OUT_OF_RANGE", "messageContent")
            }
        };
        ...
        yield return new object[]
        {
            ...
            new List<ValidationError>
            {
                Error.Validation("The 'SentAt' date cannot be in the future", "ERR_INVALID_FIELD", "SentAt")
            }
        };
    }
}
