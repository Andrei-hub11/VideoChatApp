...
public partial class Guard
{
    ...
    public IGuardInternal IsNullOrWhiteSpace(
        string value,
    ...
    )
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            ErrorList.Add(
                Error.Validation(
                    $"{valueExpression} cannot be null or empty",
    ...
    public IGuardInternal MaxLength(
        string value,
        int max,
        [CallerArgumentExpression(nameof(value))] string valueExpression = ""
    )
    {
        if (value.Length > max)
        {
            ErrorList.Add(
                Error.Validation(
                    $"{valueExpression} must have a maximum of {max} characters",
                    "ERR_TOO_LONG",
                    valueExpression
                )
            );
    ...
    public IGuardInternal MinLength(
        string value,
        int min,
        [CallerArgumentExpression(nameof(value))] string valueExpression = ""
    )
    {
        if (value.Length < min)
        {
            ErrorList.Add(
                Error.Validation(
                    $"{valueExpression} must have at least {min} characters",
                    "ERR_TOO_SHORT",
                    valueExpression
                )
            );
    ...
    )
    {
        if (value.Length < min || value.Length > max)
        {
            ErrorList.Add(
                Error.Validation(
                    $"{valueExpression} must be between {min} and {max} characters",
    ...
    public IGuardInternal AllNumeric(
        string value,
        [CallerArgumentExpression(nameof(value))] string valueExpression = ""
    )
    {
        if (!NumericRegex.IsMatch(value))
        {
            ErrorList.Add(
                Error.Validation(
                    $"{valueExpression} must contain only numbers",
                    "ERR_NOT_NUMERIC",
                    valueExpression
                )
            );
    ...
    public IGuardInternal IsAlphanumeric(
        string value,
        [CallerArgumentExpression(nameof(value))] string valueExpression = ""
    )
    {
        if (!AlphanumericRegex.IsMatch(value))
        {
            ErrorList.Add(
                Error.Validation(
                    $"{valueExpression} must contain only letters and numbers",
